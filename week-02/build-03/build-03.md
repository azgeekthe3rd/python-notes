...
```
As you can see, the output shows that the final element was not included. You can also use this example with string methods. Here is an example:
...
Now you're probably asking yourself why this is useful. We already have a couple of methods that do everything you probably need. However, if you're working with a large list where you might make a mistake trying to count the position of a chosen element manually, you will need to use the `.remove()` method. This lets you specify the element you want removed just by name. Here is an example:
